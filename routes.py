...
def index():
    ...
    sessions = SimulationSession.query.order_by(
        SimulationSession.created_at.desc()).limit(10).all()
...
def upload_file():
    ...
    try:
        ...
        if not allowed_file(file.filename):
            return jsonify(
                {'error':
                 'Invalid file type. Only XES files are supported'}), 400
        ...
        # Create session
        session = SimulationSession(filename=filename,
...
def discover_parameters(session_id):
    ...
    try:
        ...
        process_model = prosit.discover_process_model(filepath,
                                                      session.noise_threshold)
...
def update_parameters(session_id):
    ...
    try:
        ...
        required_sections = [
            'transition_params', 'resource_params', 'execution_time_params',
            'waiting_time_params'
        ]
        for section in required_sections:
            if section not in new_parameters:
                return jsonify(
                    {'error': f'Missing required section: {section}'}), 400
...
def simulate(session_id):
    ...
    try:
        ...
        if num_instances < 1 or num_instances > 10000:
            return jsonify(
                {'error':
                 'Number of instances must be between 1 and 10000'}), 400
        ...
        if start_timestamp_str:
            ...
            start_timestamp = datetime.fromisoformat(
                start_timestamp_str.replace('Z', '+00:00'))
        else:
            start_timestamp = None

        # Load process model and run simulation using ProSiT
        logger.info(
            f"Starting simulation for session {session_id} with {num_instances} instances"
        )
        ...
        result_df = prosit.run_simulation(n_traces=num_instances,
        ...
        return jsonify({
            ...
            len(result_df),
            'message':
            f'Simulation completed with {num_instances} instances, generated {len(result_df)} events'
        })
...
def upload_pnml():
    ...
    try:
        ...
        if not allowed_pnml_file(file.filename):
            return jsonify(
                {'error':
                 'Invalid file type. Only PNML files are supported'}), 400
...
def process_discovery():
    ...
    try:
        ...
        if not (event_log_file.filename
                and event_log_file.filename.lower().endswith('.xes')):
            return jsonify({'error':
                            'Please upload a XES event log file'}), 400

        # Create session
        noise_threshold = float(request.form.get(
            'noise_threshold', 0.2)) if model_source == 'discover' else 0.0
        session = SimulationSession(filename=event_log_file.filename,
                                    noise_threshold=noise_threshold,
                                    status='uploaded')
        db.session.add(session)
        db.session.commit()
        ...
        # Save event log file
        event_log_filename = secure_filename(
            f"{session.id}_{event_log_file.filename}")
        event_log_filepath = os.path.join(app.config['UPLOAD_FOLDER'],
                                          event_log_filename)
        ...
        if model_source == 'discover':
            ...
            parameters = integration.discover_process_model(
                event_log_filepath, noise_threshold)
        else:
            # Use uploaded PNML file for process model
            if 'pnml_file' not in request.files:
                return jsonify(
                    {'error': 'No PNML file provided for upload option'}), 400

            pnml_file = request.files['pnml_file']
            if pnml_file.filename == '':
                return jsonify({'error': 'No PNML file selected'}), 400

            if not (pnml_file.filename
                    and pnml_file.filename.lower().endswith('.pnml')):
                return jsonify({'error': 'Please upload a PNML file'}), 400

            # Save PNML file
            pnml_filename = secure_filename(
                f"{session.id}_{pnml_file.filename}")
            pnml_filepath = os.path.join(app.config['UPLOAD_FOLDER'],
                                         pnml_filename)
            pnml_file.save(pnml_filepath)

            # Extract parameters using PNML model and event log
            parameters = integration.extract_parameters_from_pnml_and_log(
                pnml_filepath, event_log_filepath)
        ...
        return jsonify({
            ...
            parameters,
            'process_model':
            parameters.get('process_model', {}),
            'message':
            f'Process discovery completed successfully using {model_source} method'
        })
...
def discover_model():
    ...
    try:
        ...
        session = SimulationSession.query.order_by(
            SimulationSession.created_at.desc()).first()

        if not session:
            return jsonify({
                'error':
                'No XES event log file found. Please upload an XES file first.'
            }), 400
        ...
        if not os.path.exists(filepath):
            return jsonify({
                'error':
                'Event log file not found. Please upload an XES file first.'
            }), 400
        ...
        logger.info(
            f"Process model discovered with noise threshold {noise_threshold}")
        return jsonify({
            'success':
            True,
            'session_id':
            ...
            parameters,
            'message':
            f'Process model discovered successfully with noise threshold {noise_threshold}'
        })
